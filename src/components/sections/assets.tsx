...
export function AssetsSection() {
  ...
  const scenarioType = useAtlasStore(
    (state) => state.scenario.scenarioType,
  );
  const assets =
    getInfrastructureAssets(
      scenarioType,
  ...
  return (
    <section className="workspace-page">
      ...
      <div className="asset-summary-grid">
        <Summary label="Registered assets" value={String(assets.reduce((sum, asset) => sum + asset.quantity, 0))} icon={MapPin} />
        <Summary label="Operational" value={String(assets.filter((asset) => asset.status !== "Unavailable").reduce((sum, asset) => sum + asset.quantity, 0))} icon={CheckCircle2} />
